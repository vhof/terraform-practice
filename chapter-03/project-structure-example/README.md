...
#### `terraform`
The top-level `terraform` block allows us to configure Terraform behavior, such as version and backend. The backend refers to where we store our state file, which is a remote S3 bucket in this case. By default, the state file is stored locally. 

#### `templatefile()`
To improve code clarity, we moved the launch script of our webserver instances to a seperate file, [user-data.sh](/chapter-03/project-structure-example/stage/services/webserver-cluster/user-data.sh). Using the the `templatefile()` function, we can still use Terraform string interpolation. The function takes a path string and the values for interpolation as an object (an object is similar to JavaScript / JSON objects, being a simple flexible set of key-value pairs). 